...
from tunetables.utils import set_locals_in_self
...
def get_batch_to_dataloader(get_batch_method_):
    class DL(PriorDataLoader):
        ...
        def gbm(*args, eval_pos_seq_len_sampler, **kwargs):
            ...
            if 'dynamic_batch_size' in kwargs and kwargs['dynamic_batch_size'] > 0 and kwargs['dynamic_batch_size']:
                kwargs['batch_size'] = kwargs['batch_size'] * math.floor(
                    math.pow(kwargs['seq_len_maximum'],
                             kwargs['dynamic_batch_size']) / math.pow(kwargs['seq_len'],
                                                                      kwargs[
                                                                          'dynamic_batch_size']))
        ...
        def __iter__(self):
            ...
            self.epoch_count += 1
            return iter(self.gbm(**self.get_batch_kwargs, epoch=self.epoch_count - 1, model=self.model) for _ in
...
trunc_norm_sampler_f = lambda mu, sigma: lambda: \
    stats.truncnorm((0 - mu) / sigma, (1000000 - mu) / sigma, loc=mu, scale=sigma).rvs(1)[0]
