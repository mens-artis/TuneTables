from tunetables.utils import remove_outliers  # , torch_nanmean, torch_nanstd
...
def old_torch_nanstd(x, axis=0):
    ...
    return torch.sqrt(
        torch.sum(torch.where(torch.isnan(x), torch.full_like(x, 0), torch.square(mean_broadcast - x)), dim=axis) / (
                    num - 1))
