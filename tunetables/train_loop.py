...
def train_function(config_sample, i=0, add_name='', is_wrapper=False, x_wrapper=None, y_wrapper=None, cat_idx=[]):
    ...
    def save_callback(model, epoch, values_to_log):
        ...
        if len(epochs) % save_every_k == 0:
            ...
            save_model(model, config_sample['base_path'],
    ...
    model, results_dict, data_for_fitting, test_loader = get_model(config_sample
                                                                   ...
                                                                   , epoch_callback=my_callback, is_wrapper=is_wrapper,
                                                                   x_wrapper=x_wrapper, y_wrapper=y_wrapper,
                                                                   cat_idx=cat_idx)
...
def set_compatibility_params(config, args_parsed_from_cmdline):
    ...
    config['eval_positions'] = [int(config['bptt'] * 0.95)] if config['bptt_extra_samples'] is None else [
        int(config['bptt'])]
    # ?
...
def reload_config(config_type='causal', task_type='multiclass', longer=0,
    ...
    if config['uniform_bptt']:
        ...
        # if config['bptt'] < 128:
        #     print("Warning: bptt should be >= 128 when using uniform bptt, as currently 128 samples per batch are
        #     reserved for evaluation. Setting bptt to 128.")
        #     config['bptt'] = 128
...
def parse_args():
    ...
    parser.add_argument('--resume', type=str, default="../models/prior_diff_real_checkpoint_n_0_epoch_42.cpkt",
    ...
    parser.add_argument('--tuned_prompt_label_balance', type=str, default='equal',
                        help='Label balance for the tuned prompt (equal, proportional).')
    parser.add_argument('--lr', type=float, default=0.0001, help='Learning rate.')
    parser.add_argument('--batch_size', type=int, default=1, help='Batch size.')
    parser.add_argument('--bptt', type=int, default=1152, help='Batch per train time.')
    parser.add_argument('--bptt_search', action='store_true',
                        help='Search for the near-maximum bptt that will fit in memory in range (32, 65536).')
    parser.add_argument('--uniform_bptt', action='store_true',
                        help='Whether to use uniform bptt. Note that uniform bptt adds 128 extra samples per batch (for evaluation), so bptt should be > 128 when using uniform_bptt.')
    ...
    parser.add_argument('--num_eval_fitting_samples', type=int, default=1000,
                        help='How many samples from the training set to draw when fitting the eval set.')
    ...
    parser.add_argument('--subsampling', type=int, default=0,
                        help='Qty of data to subsample during training (0 = no subsampling).')
    parser.add_argument('--rand_init_ensemble', action='store_true', help='Ensemble over random initialization.')
    parser.add_argument('--ensemble_lr', type=float, default=0.5,
                        help='Additive learning factor for boosting / ensembling.')
    parser.add_argument('--ensemble_size', type=int, default=5, help='Number of ensemble members.')
    parser.add_argument('--reseed_data', action='store_true',
                        help='Whether to randomly rotate features, labels and fitting data in the ensemble.')
    ...
    parser.add_argument('--permute_feature_position_in_ensemble', action='store_true',
                        help='Whether to ensemble over feature position permutations.')
    ...
    parser.add_argument('--subset_features_method', type=str, default='mutual_information',
                        help='Method for feature subset selection ("mutual_information, random, first, pca").')
    parser.add_argument('--subset_features', type=int, default=100,
                        help='Number of features to use for feature subset selection.')
    parser.add_argument('--pad_features', action='store_true',
                        help='Whether to pad features to the maximum number of features.')
    parser.add_argument('--do_preprocess', action='store_true',
                        help='Whether to add tabpfn-style preprocessing to the data.')
    ...
    parser.add_argument('--keep_topk_ensemble', type=int, default=0,
                        help='Whether to keep only the top-k ensemble members.')
    ...
    parser.add_argument('--preprocess_type', type=str, default='none',
                        help='Type of preprocessing to use (none, power_all, quantile_all, robust_all).')
    ...
    parser.add_argument('--shuffle_every_epoch', action='store_true',
                        help='Whether to shuffle the order of the data every epoch (can help when bptt is large).')
